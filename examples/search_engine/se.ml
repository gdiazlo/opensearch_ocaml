...
type requester =
  ...
  -> Req.t
  -> (Cohttp_eio__.Body.t, string) result
...
let format_cvss json =
  ...
  let confidentialityImpact =
    cvss |> member "confidentialityImpact" |> to_string_even_null
  in
...
let format_result res =
  ...
  List.map
    (fun hit ->
      ...
      let title =
        parse_or_default
          (fun () -> cna |> member "title" |> to_string_even_null)
          "No title available"
      in
      let cvss =
        parse_or_default
          (fun () -> cna |> member "metrics" |> to_list |> List.hd |> format_cvss)
          cvss_default
      in
...
let search (osc : requester) query =
  ...
  let params =
    Param.empty |> with_q query |> with_size 10 |> with_timeout (1 * Duration.second)
  in
  Eio.Switch.run
  @@ fun sw ->
  ...
  | Ok b ->
    Ok
      (Eio.Buf_read.(parse_exn take_all) b ~max_size:max_int
...
let format_cvss cvss =
  ...
  details
    ...
    ; ul
        ...
        [ li
            []
            [ span [] [ txt "Attack complexity" ]
            ; span [] [ txt "%s" cvss.attackComplexity ]
            ]
        ; li
            []
            [ span [] [ txt "Attack vector" ]; span [] [ txt "%s" cvss.attackVector ] ]
        ; li
            []
            [ span [] [ txt "Availability impact" ]
            ; span [] [ txt "%s" cvss.availabilityImpact ]
            ]
        ; li [] [ span [] [ txt "Base score" ]; span [] [ txt "%f" cvss.baseScore ] ]
        ; li
            []
            [ span [] [ txt "Base severity" ]; span [] [ txt "%s" cvss.baseSeverity ] ]
        ; li
            []
            [ span [] [ txt "Confidentiality impact" ]
            ; span [] [ txt "%s" cvss.confidentialityImpact ]
            ]
        ; li
            []
            [ span [] [ txt "Integrity impact" ]
            ; span [] [ txt "%s" cvss.integrityImpact ]
            ]
        ; li
            []
            [ span [] [ txt "Privileges required" ]
            ; span [] [ txt "%s" cvss.privilegesRequired ]
            ]
        ; li [] [ span [] [ txt "Scope" ]; span [] [ txt "%s" cvss.scope ] ]
        ; li
            []
            [ span [] [ txt "User interaction" ]
            ; span [] [ txt "%s" cvss.userInteraction ]
            ]
        ; li
            []
            [ span [] [ txt "Vector string" ]; span [] [ txt "%s" cvss.vectorString ] ]
...
let format_cve cve =
  ...
  article
    ...
    ; div
        [ class_ "grid" ]
        [ div [] [ p [] [ txt "%s" cve.cveId ]; hr []; p [] [ txt "%s" cve.description ] ]
        ; div [] [ format_cvss cve.cvss ]
        ]
    ; footer
        []
...
let format_cve_list cves =
  let open Dream_html in
  let open HTML in
  div [] [ ul [] (List.map (fun cve -> li [] [ format_cve cve ]) cves) ]
;;
...
let error_page str =
  ...
  article
    []
    [ header [] [ h1 [] [ txt "Error" ] ]; pre [] [ txt "%s" str ]; footer [] [] ]
;;
...
let results_page cves =
  ...
  article
    []
    [ header [] [ h1 [] [ txt "Search results" ] ]; format_cve_list cves; footer [] [] ]
;;
...
let page n req =
  ...
  html
    [ lang "en" ]
    [ head [] [ title [] "Search CVE engine" ]
    ; link
        [ rel "stylesheet"
        ; href "https://cdn.jsdelivr.net/npm/@picocss/pico@2/css/pico.pumpkin.min.css"
        ]
    ; link [ rel "stylesheet"; href "static/styles.css" ]
    ; body [] [ main [ class_ "container" ] [ div [] [ search_bar req ]; div [] [ n ] ] ]
    ]
;;
...
let search_handler req osc =
  ...
  | `Ok [ ("query", q) ] ->
    ...
    (match resp with
     | Ok cves -> Dream_html.respond (page (results_page cves) req)
