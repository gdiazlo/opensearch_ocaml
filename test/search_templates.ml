...
let index_create (do_req : requester) =
  ...
  let res = do_req (Api.Index.Create.put "search_template_index_test" params) in
  let _ =
    match res with
    | Ok _ -> Alcotest.pass
...
let index_exists (do_req : requester) =
  let params = Param.empty in
  let res = do_req (Api.Index.Exists.head "search_template_index_test" params) in
  let _ =
    match res with
    | Ok _ -> Alcotest.pass
...
let bulk_create (do_req : requester) =
  ...
  let body =
    Bulk.to_body
      (Bulk.init 100 (fun i ->
         Bulk.with_create "search_template_index_test" (`Assoc [ "data", `Int i ])))
  in
...
let search (do_req : requester) =
  ...
  in
  let res = do_req ~body (Api.Search_templates.search params) in
  let _ =
    match res with
    | Ok _ -> Alcotest.pass
...
let index_delete (do_req : requester) =
  let params = Param.empty in
  let res = do_req (Api.Index.Delete.delete "search_template_index_test" params) in
  let _ =
    match res with
    | Ok _ -> Alcotest.pass
