...
let index_create (do_req : requester) =
  ...
  let res = do_req (Api.Index.Create.put "search_index_test" params) in
  let _ =
    match res with
    | Ok _ -> Alcotest.pass
...
let index_exists (do_req : requester) =
  let params = Param.empty in
  let res = do_req (Api.Index.Exists.head "search_index_test" params) in
  let _ =
    match res with
    | Ok _ -> Alcotest.pass
...
let bulk_create (do_req : requester) =
  ...
  let body =
    Bulk.to_body
      (Bulk.init 100 (fun i ->
         Bulk.with_create "search_index_test" (`Assoc [ "data", `Int i ])))
  in
...
let search (do_req : requester) =
  let params = Param.empty |> Api.Search.with_size 1 in
  let body = Cohttp_eio.Body.of_string {| { "query": { "term": { "data": 4  } } }|} in
  let res = do_req ~body (Api.Search.post "search_index_test" params) in
  let _ =
    match res with
    | Ok _ -> Alcotest.pass
    | Error str -> Alcotest.failf "search error: %s" str
  in
  ()
;;
...
let index_delete (do_req : requester) =
  let params = Param.empty in
  let res = do_req (Api.Index.Delete.delete "search_index_test" params) in
  let _ =
    match res with
    | Ok _ -> Alcotest.pass
  ...
  Alcotest.run
    ...
    [ ( "opensearch client"
      , [ test_case "index_create" index_create do_req
        ...
        ; test_case "search" search do_req
